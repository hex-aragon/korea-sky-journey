...
test('all Blender GLB assets are valid self-contained models',async()=>{const {readFile}=await import('node:fs/promises');for(const name of ['fighter','skywing','glider','namsan','pavilion','lighthouse','broadleaf','seoul-tower']){const b=await readFile(new URL(`../public/models/${name}.glb`,import.meta.url));assert.equal(b.toString('ascii',0,4),'glTF');assert.equal(b.readUInt32LE(4),2);assert.equal(b.readUInt32LE(8),b.length);const len=b.readUInt32LE(12),json=JSON.parse(b.toString('utf8',20,20+len));assert(json.meshes.length>0);assert(json.buffers.every(buffer=>!buffer.uri));assert(!json.images?.some(image=>image.uri?.startsWith('http')));}});
