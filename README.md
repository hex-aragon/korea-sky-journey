...
- 오른쪽 위 북쪽 고정 미니맵: 현재 위치/기수 방향, 주변 도시, 레이싱 경로와 다음 링. 메뉴와 풍경 모드에서는 숨깁니다.
- 힐링·전투 외에 **하늘 레이싱**: 12개 링, 순서대로 통과, 연속 통과 점수. 주황 비콘 접촉은 +3초, 놓친 링은 +5초입니다. 15초 동안 링에 도달하지 못해도 다음 링으로 계속됩니다.
- 종료 후 6초 뒤 다른 지역의 새 코스로 연결합니다. `편안한 비행으로`를 누르면 나갈 수 있고 P/창 전환/메뉴는 카운트다운을 멈춥니다. 최고 점수는 해당 브라우저에만 저장됩니다.
- 코스마다 지역, 고도대, 완만한 곡선, 장애물, 체험 날씨를 바꿉니다. 실제 한국 지형은 유지하며 **비행 코스와 하늘 풍경**을 생성합니다. 원본 지형을 임의의 나라처럼 바꾸지는 않습니다.
- `여행 → 새로운 하늘 만나기`는 언제든 다음 무작위 코스(힐링에서는 다른 도시와 날씨)로 이동합니다.
- 원본 절차 모델로 만든 36마리 새와 7개 열기구가 바람과 함께 움직입니다. 별도 서비스·유료 API·GPU 서버가 필요하지 않습니다. 새/열기구는 감상용이며 레이싱 충돌은 주황 장애물에만 적용됩니다.
- 참고: [FAI 열기구 비행](https://www.fai.org/sport/ballooning), [NPS 상승 기류와 활공](https://www.nps.gov/articles/000/riding-the-winds-23.htm). 해당 사이트의 이미지·모델을 복제하지 않고 풍경 연출을 참고했습니다.

`tests/race.test.mjs`는 1,000개 시드의 경계/지형 여유/링과 장애물 분리, 고속 통과, 중복 점수 방지, 충돌 유예, 중단/재시작, 시간 초과를 검증합니다.

### 직접 조준과 전투 난이도

전투 화면을 마우스 또는 손가락으로 드래그하면 조준점만 이동합니다. 기체 조종은 기존 키/버튼을 사용합니다. 적기에 조준점을 유지해 잠금을 완료한 뒤 `미사일 발사` 또는 F로 발사하고, `기수로 조준`으로 기존 조준 방식에 복귀합니다.

시작 화면과 여행 메뉴에서 전투 난이도를 선택합니다(기본 하). 변경 시 전투 점수와 적기를 초기화합니다.

| 난이도 | 조준 완료 | 조준 허용 각도 | 적 속도 배율 | 회피 시간 |
|---|---:|---:|---:|---:|
| 하 | 0.35초 | 18° | 0.65 | 5초 |
| 중 | 0.75초 | 10° | 1 | 3초 |
| 상 | 1초 | 6° | 1.2 | 2.5초 |
