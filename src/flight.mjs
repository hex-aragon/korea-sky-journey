...
export function stepFlight(state,input,dt,wind,ground){
 ...
 state.pitch=damp(state.pitch,input.climb*.2,2,dt);state.speed=damp(state.speed,input.fast?160:Math.max(30,Math.min(120,input.cruise??55)),1,dt);
...
export function approachAltitude(current,target,dt,rate=120){const delta=Math.max(-rate*dt,Math.min(rate*dt,target-current));return current+delta;}
