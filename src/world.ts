...
import { loadTerrain, terrainHeight, coastalHeightTexture } from './terrain';
import { aircraftById } from './aircraft.mjs';
...
export class World{
 airport={x:0,y:0,z:0};
 scene=new THREE.Scene();sky=new Sky();sun=new THREE.DirectionalLight('#fff1d4',2.3);hemi=new THREE.HemisphereLight('#c8e8ff','#7c9c72',2.4);ocean:THREE.Mesh;sunDirection=new THREE.Vector3(-.6,.4,-.6).normalize();aircraft=new THREE.Group();airframe=new THREE.Group();private aircraftCache=new Map<string,Promise<THREE.Group>>();private aircraftRequest=0;landmarks:THREE.Group[]=[];obstacles:{x:number;z:number;w:number;d:number;h:number}[]=[];rain:THREE.LineSegments;snow:THREE.Points;stars:THREE.Points;moon:THREE.Mesh;private cityMaterial:THREE.MeshStandardMaterial|undefined;ready:Promise<void>;cameraCity=0;private time=0;private obstacleGrid:ObstacleGrid|undefined;
 constructor(){
  ...
  this.aircraft.add(this.airframe);this.scene.add(this.aircraft);this.ready=this.initialize();
 }
 async initialize(){
  ...
  await loadTerrain(this.scene);(this.ocean.material as THREE.ShaderMaterial).uniforms.coastalHeights.value=coastalHeightTexture();this.createCities();this.airport=createScenery(this.scene,this.obstacles).airport;this.createLandmarks();await this.loadModels();await this.createForest();this.obstacleGrid=new ObstacleGrid(this.obstacles);
 }
 async createForest(){
 ...
 createCities(){
  ...
  cities.forEach((c,ci)=>{const center=positions[ci];this.createDistrict(center.x,center.z);const count=ci===0?2400:ci===6?1500:ci===1?1200:650;const occupied=new Set<string>();
   for(let i=0;i<count;i++){const x=center.x+(random()-.5)*1050,z=center.z+(random()-.5)*950;if(!isLand(x,z))continue;const gx=Math.round((x-center.x)/22)*22+center.x,gz=Math.round((z-center.z)/22)*22+center.z;const cell=`${gx},${gz}`;if(occupied.has(cell)||!isLand(gx,gz))continue;occupied.add(cell);if([gx-center.x,gz-center.z].some(v=>Math.abs(v-Math.round(v/125)*125)<12))continue;const core=Math.max(0,1-Math.hypot(gx-center.x,gz-center.z)/460),tower=random()<core*.3;const w=tower?10+random()*7:9+random()*8,d=tower?10+random()*7:8+random()*10,h=tower?22+random()*core*(ci===0||ci===6?62:30):3+random()**2*13;const y=terrainHeight(gx,gz);if(y>90||Math.abs(terrainHeight(gx+10,gz)-y)>9)continue;items.push({x:gx,y,z:gz,w,h,d,c:['#b3b9b5','#969fa0','#6d8c99','#b6b0a3','#88979c'][Math.floor(random()*5)]});this.obstacles.push({x:gx,z:gz,w:w/2,d:d/2,h:y+h});}
  });
 ...
 async loadModels(){const loader=new GLTFLoader();const base=import.meta.env.BASE_URL;await this.selectAircraft('kestrel');this.aircraft.scale.setScalar(2.2);this.aircraft.traverse(o=>{if(o instanceof THREE.Mesh){o.castShadow=true;o.receiveShadow=true;const mats=Array.isArray(o.material)?o.material:[o.material];for(const m of mats){if(m instanceof THREE.MeshStandardMaterial){m.emissive.copy(m.color);m.emissiveIntensity=.055;}}}});const load=async(file:string,indexes:number[])=>{const data=await loader.loadAsync(base+'models/'+file+'.glb');for(const i of indexes)this.landmarks[i].add(data.scene.clone());};const tower=await loader.loadAsync(base+'models/seoul-tower.glb');const tp=geo(127.1025,37.5126);tower.scene.position.set(tp.x,terrainHeight(tp.x,tp.z),tp.z);tower.scene.traverse(o=>{if(o instanceof THREE.Mesh){o.castShadow=true;o.receiveShadow=true;}});this.scene.add(tower.scene);this.obstacles.push({x:tp.x,z:tp.z,w:22,d:22,h:tower.scene.position.y+205});await Promise.all([load('namsan',[0]),load('pavilion',[3,5,8]),load('lighthouse',[2,7,9])]);}
 async selectAircraft(id:string){
  const choice=aircraftById(id);if(!choice)throw new Error('Unknown aircraft');const request=++this.aircraftRequest;
  let pending=this.aircraftCache.get(id);if(!pending){pending=new GLTFLoader().loadAsync(import.meta.env.BASE_URL+'models/'+choice.file).then(data=>{data.scene.traverse(o=>{if(o instanceof THREE.Mesh){o.castShadow=true;o.receiveShadow=true;}});return data.scene;});this.aircraftCache.set(id,pending);pending.catch(()=>this.aircraftCache.delete(id));}
  const model=await pending;if(request!==this.aircraftRequest)return false;this.airframe.clear();this.airframe.add(model);return true;
 }
